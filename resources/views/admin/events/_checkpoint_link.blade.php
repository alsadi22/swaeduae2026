<div class="mt-8 overflow-hidden border border-gray-200 bg-gray-50/80 shadow-sm sm:rounded-lg">
    ...
    <div class="space-y-4 p-6 text-sm text-gray-700">
        ...
        <form method="post" action="{{ route($checkpointSignedUrlRoute ?? 'admin.events.checkpoint-signed-url', array_merge(['event' => $event], $checkpointRouteQuery ?? $adminLocaleQ ?? [])) }}" class="flex flex-wrap items-end gap-3 border-t border-gray-200 pt-4">
            @csrf
            <div>
