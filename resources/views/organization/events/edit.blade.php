<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-wrap items-center justify-between gap-4">
            ...
            <a href="{{ route('organization.events.roster', $event) }}" class="text-sm font-medium text-indigo-600 hover:text-indigo-900">
                {{ __('View roster') }}
            </a>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <form method="post" action="{{ route('organization.events.update', $event) }}" class="p-6 space-y-6">
                    @csrf
                    @method('put')
