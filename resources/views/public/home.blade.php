    ...
    <section class="relative overflow-hidden bg-gradient-to-br from-slate-950 via-emerald-950 to-teal-950 text-white">
        ...
        <div class="relative mx-auto max-w-content px-4 py-14 sm:px-6 sm:py-20 lg:py-24">
            ...
            <div class="mt-10 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
                ...
                <a href="{{ route('youth-councils', $homeLocaleQ) }}" class="btn-hero-ghost">{{ __('Youth Councils') }}</a>
                <a href="{{ route('volunteer.index', $homeLocaleQ) }}" class="btn-hero-ghost">{{ __('Become a volunteer') }}</a>
                <a href="{{ route('events.index', $homeLocaleQ) }}" class="btn-hero-outline">{{ __('View events') }}</a>
                <a href="{{ route('contact.show', $homeLocaleQ) }}" class="btn-hero-outline">{{ __('Partner with us') }}</a>
            </div>
        </div>
    </section>
    ...
    <section class="border-b border-slate-200 bg-white" aria-labelledby="start-here-heading">
        <div class="mx-auto max-w-content px-4 py-10 sm:px-6">
            ...
            <div class="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <a href="{{ route('youth-councils', $homeLocaleQ) }}" class="card-interactive group flex gap-4">
                    <span class="card-icon-tile" aria-hidden="true">
                ...
                <a href="{{ route('events.index', $homeLocaleQ) }}" class="card-interactive group flex gap-4">
                    <span class="card-icon-tile" aria-hidden="true">
                ...
                <a href="{{ route('volunteer.index', $homeLocaleQ) }}" class="card-interactive group flex gap-4">
                    <span class="card-icon-tile" aria-hidden="true">
                ...
                <a href="{{ route('media.index', $homeLocaleQ) }}" class="card-interactive group flex gap-4">
                    <span class="card-icon-tile" aria-hidden="true">
                ...
                <a href="{{ route('gallery', $homeLocaleQ) }}" class="card-interactive group flex gap-4">
                    <span class="card-icon-tile" aria-hidden="true">
                ...
                <a href="{{ route('contact.show', $homeLocaleQ) }}" class="card-interactive group flex gap-4">
                    <span class="card-icon-tile" aria-hidden="true">
    ...
    <div class="mx-auto max-w-content space-y-20 px-4 py-16 sm:px-6 sm:py-20">
        ...
        <section class="card-surface p-8 sm:p-10">
            ...
            <div class="mt-8 flex flex-wrap gap-4">
                <a href="{{ route('about', $homeLocaleQ) }}" class="inline-flex text-sm font-bold text-emerald-800 hover:text-emerald-950">{{ __('Read more') }} →</a>
                <a href="{{ route('cms.page', array_merge($homeLocaleQ, ['slug' => 'community-charter'])) }}" class="inline-flex text-sm font-bold text-slate-600 hover:text-emerald-900">{{ __('Community charter') }} →</a>
            </div>
        </section>
        ...
        <section>
            <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
                ...
                <a href="{{ route('programs.index', $homeLocaleQ) }}" class="shrink-0 text-sm font-bold text-emerald-800 hover:underline">{{ __('View all') }}</a>
            </div>
                ...
                @forelse ($featuredCmsPages as $fp)
                    <article class="card-surface overflow-hidden transition duration-200 hover:shadow-card-hover">
                        ...
                        <div class="p-5">
                            ...
                            <a href="{{ $fp->absolutePublicUrl() }}" class="mt-4 inline-flex text-sm font-bold text-emerald-800 hover:underline">{{ __('Read more') }} →</a>
                        </div>
                    </article>
                @empty
            ...
            <div class="grid gap-10 lg:grid-cols-2 lg:items-center">
                <div>
                    ...
                    <div class="mt-8 flex flex-wrap gap-3">
                        <a href="{{ route('volunteer.index', $homeLocaleQ) }}" class="btn-primary-solid">{{ __('Open volunteer hub') }}</a>
                        @guest
                            <a href="{{ route('register.volunteer', $homeLocaleQ) }}" class="btn-secondary-muted">{{ __('Create account') }}</a>
                        @endguest
                    </div>
                </div>
        ...
        <section>
            <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
                ...
                <a href="{{ route('events.index', $homeLocaleQ) }}" class="shrink-0 text-sm font-bold text-emerald-800 hover:underline">{{ __('Calendar') }}</a>
            </div>
            <ul class="mt-10 space-y-4">
                @forelse ($upcomingEvents as $ev)
                    <li class="card-surface flex flex-wrap items-center justify-between gap-4 px-5 py-4" data-testid="home-upcoming-event">
                        ...
                        <a href="{{ route('events.show', array_merge(['event' => $ev], $homeLocaleQ)) }}" class="shrink-0 text-sm font-bold text-emerald-800 hover:text-emerald-950 hover:underline">{{ __('Details') }}</a>
                    </li>
                @empty
                    <li class="card-surface px-5 py-4">
                        <p class="text-sm text-slate-600">{{ __('No upcoming events listed yet.') }}</p>
                        <div class="mt-3 flex flex-wrap gap-4">
                            <a href="{{ route('events.index', $homeLocaleQ) }}" class="text-sm font-bold text-emerald-800 hover:underline">{{ __('Calendar') }} →</a>
                            <a href="{{ route('volunteer.opportunities.index', $homeLocaleQ) }}" class="text-sm font-bold text-emerald-800 hover:underline">{{ __('Browse opportunities') }} →</a>
                        </div>
                    </li>
                @endforelse
            </ul>
        ...
        <section>
                    ...
                    @foreach ($homeNewsTeasers as $row)
                        @if ($row['kind'] === 'internal')
                            ...
                            <li class="card-surface p-5 transition duration-200 hover:shadow-card-hover">
                                ...
                                <a href="{{ $item->absolutePublicUrl() }}" class="mt-1 block font-display font-bold text-emerald-900 hover:underline">{{ $item->title }}</a>
                                @if ($item->excerpt)
                        ...
                        @else
                            ...
                            <li class="card-surface p-5 transition duration-200 hover:shadow-card-hover ring-1 ring-amber-100/80">
                                ...
                                <a href="{{ $ext->absolutePublicUrl() }}" class="mt-1 block font-display font-bold text-emerald-900 hover:underline">{{ $ext->titleForLocale() }}</a>
                                ...
                                <div class="mt-3 flex flex-wrap gap-4 text-sm font-bold">
                                    <a href="{{ $ext->absolutePublicUrl() }}" class="text-emerald-800 hover:underline">{{ __('View details') }}</a>
                                    @if ($ext->external_url)
            ...
            @endif
            <a href="{{ route('media.index', $homeLocaleQ) }}" class="mt-6 inline-flex text-sm font-bold text-emerald-800 hover:underline">{{ __('Media center') }} →</a>
        </section>

        {{-- Partners strip (config `swaeduae.home_partners` — see config/swaeduae.php) --}}
        @php
            $homePartners = config('swaeduae.home_partners', []);
        @endphp
        <section class="@if (count($homePartners) > 0) rounded-2xl border border-slate-200/90 bg-white px-6 py-10 shadow-sm ring-1 ring-slate-100 sm:px-10 @else card-dashed-placeholder @endif" aria-labelledby="home-partners-heading">
