...
<x-public-layout :title="$pageTitle" :metaDescription="$metaDescription">
    <div class="mx-auto max-w-content px-4 py-12 sm:px-6 sm:py-16">
        <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
            ...
            <a href="{{ route('volunteer.index') }}" class="shrink-0 text-sm font-bold text-emerald-800 hover:underline">{{ __('Volunteer hub') }} →</a>
        </div>
        ...
        <form method="get" action="{{ route('volunteer.opportunities.index') }}" class="card-surface mt-10 flex flex-col gap-4 p-4 sm:flex-row sm:flex-wrap sm:items-end">
            ...
            <div class="flex flex-wrap gap-2">
                ...
                <a href="{{ route('volunteer.opportunities.index') }}" class="btn-secondary-muted">{{ __('Clear') }}</a>
            </div>
        </form>
        ...
        @if ($events->total() === 0)
            <div class="card-dashed-placeholder mt-12">
                @if (filled($search) && $entry === 'all')
                    ...
                    <a href="{{ route('volunteer.opportunities.index', array_filter(['sort' => $sort !== 'starts_soon' ? $sort : null])) }}" class="btn-secondary-muted mt-6 inline-flex">{{ __('Clear') }}</a>
                ...
                @foreach ($events as $ev)
                    <li class="card-surface p-6">
                        <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
                            ...
                            <a href="{{ route('volunteer.opportunities.show', $ev) }}" class="btn-primary-solid shrink-0 text-center">{{ __('View details') }}</a>
                        </div>
                    </li>
                @endforeach
            </ul>
